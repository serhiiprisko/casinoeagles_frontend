  ...
  <div class="main-container bg-primary" :class="($mq==='xl' || $mq==='lg') ? 'desktop' : 'mobile'">
    <b-container v-if="!getGamePlayStatus" fluid class="mx-0">
      ...
      <b-row class="px-0">
        <b-col class="bg-dark">
          <b-nav
            class="game-group-nav"
            :class="($mq==='xl' || $mq==='lg') ? 'd-flex justify-content-center align-items-center' : ''"
            :style="($mq==='xl' || $mq==='lg') ? '' : 'overflow-x:scroll;'"
          >
        ...
        <b-container fluid class="px-4">
          ...
          <div class="infinite-wrapper">
            <b-row v-for="(index) in Math.ceil(gameCount/gamesPerRow)" :key="index">
              <b-col v-for="(colIndex) in gamesPerRow" :key="colIndex" class="p-1" @mouseover="hoverOver((index-1)*gamesPerRow + colIndex-1)" @mouseout="hoverOut()">
                <template v-if="(index-1)*gamesPerRow+colIndex <= totalGameCount">
                  <b-row>
                    <b-col cols="12" class="h-100">
                      ...
                      />
                      <div class="game-actions d-flex justify-content-center align-items-center" v-bind:class="{ activeOp: hoverOverIndex === (index-1)*gamesPerRow + colIndex-1 }">
                        <b-row class="w-100" v-bind:class="{ activeDs: hoverOverIndex === (index-1)*gamesPerRow + colIndex-1 }">
                  ...
                  <b-row>
                    <b-col cols="12" class="d-flex justify-content-center align-items-center">
                      <h5 class="text-white">
                        {{ gameList[(index-1)*gamesPerRow + colIndex-1].name }}
                      </h5>
                    </b-col>
                  </b-row>
                </template>
    ...
    <b-container v-else class="game-container h-100 w-100" :class="($mq==='xl' || $mq==='lg') ? 'desktop' : 'mobile'">
      <b-row align-h="center" align-v="center" class="h-100" style="padding-top: 10px">
        <b-col cols="12">
          <b-row>
            <b-col
...
export default {
  ...
  computed: {
    ...mapGetters(['getGamePlayStatus', 'loggedIn', 'loggedUser', 'getCountryCode'])
  },
  watch: {
  },
  created () {
  ...
  methods: {
    ...mapActions('game', ['getGames', 'getClientIP', 'getGameURL']),
        ...
        try {
          ...
          const data = await this.getGameURL({
            ...
            params: {
              ...
              game_code: this.game.slug,
              player_id: this.loggedUser.id,
              currency: 'EUR',
              is_real: false,
              lang: this.$i18n.locale,
              ip: clientIP['ip'],
              country: clientIP['country'].toUpperCase()
            }
          })
          if (data.status === 1) {
            if (this.$mq === 'md') {
    ...
    async PlayGameAction (game, isRealPlay = false) {
      ...
      const data = await this.getGameURL({
        ...
        params: {
          ...
          game_code: this.game.slug,
          player_id: isRealPlay ? this.loggedUser.id : 'test',
          currency: 'EUR',
          is_real: isRealPlay,
          lang: this.$i18n.locale,
          ip: clientIP['ip'],
      ...
      if (data.status === 1) {
        if (this.$mq === 'md') {
          await this.setGamePlayStatus(false)
          this.popWindow.location = data.gameURL
